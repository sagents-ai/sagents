...
`Sagents.Agent.new/2` now warns when one of its options is passed in the
attributes map.
...
`Agent.new/2` reads `replace_default_middleware`, `todo_opts`,
`filesystem_opts`, `summarization_opts`, `subagent_opts`, and `interrupt_on`
from its second argument. `Ecto.Changeset.cast/3` drops keys it does not
recognize, so one of these placed in the attributes map is ignored: an agent
asked to `replace_default_middleware` gets the full default stack, and an
`interrupt_on` config never reaches `HumanInTheLoop`.

Such a call now logs a warning naming each misplaced key. **It will raise in
v0.15.0.** Move them to the second argument:

```elixir
# Warns, and replace_default_middleware has no effect
Agent.new(%{model: model, replace_default_middleware: true, middleware: [MyMiddleware]})
...
The factory generated by `mix sagents.setup` already passes its options
correctly, so a generated factory needs no change unless it was edited by hand.
To find other call sites, search for the option names:

```
grep -rnE "replace_default_middleware|todo_opts|filesystem_opts|summarization_opts|subagent_opts|interrupt_on" lib test
```

Once an option is moved it takes effect, which can change behavior. With
`replace_default_middleware` honored, the default middleware (TodoList,
FileSystem, SubAgent, Summarization, PatchToolCalls) and their tools and system
prompt text are no longer present. Tests that depended on them, such as LLM
stubs that count messages expecting a system message, may need updating. With
`interrupt_on` honored, invalid values such as `:always` fail `HumanInTheLoop`
initialization; use `true`, `false`, or a config map.

### Changed

- `Agent.new/2` logs a warning naming each option found in the attributes map.
  [#192](https://github.com/sagents-ai/sagents/pull/192)
- The `Sagents`, `Sagents.Agent`, and `Sagents.Middleware.PatchToolCalls` module
  docs show options in the second argument of `Agent.new/2`.
  [#182](https://github.com/sagents-ai/sagents/pull/182)
- The `usage_rules` dev dependency is upgraded to 1.2.8 for CVE-2026-82710 and
  configured through `mix.exs`. It is dev-only, so applications using `sagents`
  are unaffected. [#191](https://github.com/sagents-ai/sagents/pull/191)

### Fixed

- A subagent's configured `max_runs` is carried through to execution instead of
  falling back to the default of 50. The budget is checked before the next LLM
  call, so tools returned by the last permitted response still run, including an
  `until_tool` target and an approved `HumanInTheLoop` tool on resume.
  [#183](https://github.com/sagents-ai/sagents/pull/183)
- Subagents run inside the caller's OpenTelemetry context, so their spans join
  the parent trace. [#187](https://github.com/sagents-ai/sagents/pull/187)
