...
When `Sagents.Agent.resume/3` is called, the middleware stack is cycled in order. Each middleware's `handle_resume/5` gets a chance to claim the interrupt by pattern-matching on `state.interrupt_data`.
